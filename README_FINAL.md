...
> 2D transient heat transfer simulation of a Thermal Protection System (TPS) for spacecraft atmospheric reentry using finite difference method with ML acceleration
...
This project simulates the thermal behavior of a thermal protection tile used in spacecraft during atmospheric reentry. The simulation employs a **2D finite difference method (FDM)** with implicit Euler time integration and Newton-Raphson linearization for nonlinear radiation boundary conditions.
...
- **2D Transient Heat Conduction**
  ...
  - Finite difference spatial discretization (2nd order centered)
...
#### 🤖 ML Surrogate Model
- **Neural network** trained on 500 finite difference simulations
- **Performance**: R² = 0.997, MAE = 4.0°C
- **Speedup**: 47× faster than FDM (1.4s → 0.03s)
- **Use case**: Rapid design space exploration before detailed numerical validation
...
│   ├── tps_fct.py               # FDM solver functions
...
- **Time step**: 10s (adjustable, stability guaranteed by implicit scheme)
...
### ML Surrogate
- **Single prediction**: 0.03s (47× faster than FDM)
...
```
1000 designs × 1.4s = 23 minutes (numerical solver only)
```

ML-accelerated approach:
```
1000 designs × 0.03s = 30s (ML screening)
+ 10 best designs × 1.4s = 14s (numerical validation)
...
- [ ] Adaptive mesh refinement for boundary layers
...
- [ ] Integration with FEM software (COMSOL, ANSYS) for validation
...
### External Resources
- [COMSOL Multiphysics](https://www.comsol.com/) - Commercial FEM/FDM software for validation
- [NASA TPS Database](https://tps.nasa.gov/) - Material properties
- [NumPy Documentation](https://numpy.org/doc/) - Numerical computing in Python
...
@software{tps_simulation_2026,
  author = {Your Name},
  title = {TPS Thermal Shield Simulation: 2D Finite Difference Solver with ML Acceleration},
  year = {2026},
